...
class Thumbnail_Extractor implements \Iterator
{
    ...
    /**
     * @var int video length
     */
    protected $duration = 0;

    /**
     * @var bool A switch to keep track of the end of the array
     */
    private $valid = false;
     ...
     */
    public function __construct($video, $frames = [], $size = '', $ffmpeg = 'ffmpeg')
    {
    ...
    private function _getDuration()
    {
        ...
        if (preg_match('/Duration: ((\d+):(\d+):(\d+))/s', shell_exec($cmd), $time)) {
     ...
     *
     * @param int $second seconds from start
     *
     * @return string binary image contents
     */
    private function getFrame($second)
    {
        $temp = __PLEX_VAR_DIR__.'/build/'.md5($this->video);
        (new Filesystem())->mkdir($temp);
        $image = tempnam($temp, 'FRAME_');
