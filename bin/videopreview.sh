...
mkdir -p "$OUT_DIR"
# ffmpeg -i -vf "select='between(t,235,238)+between(t,490,493)+between(t,870,873)+between(t,1200,1203)',setpts=N/FRAME_RATE/TB,scale=640:-2" -an out.mp4
FFMPEG_CMD="ffmpeg -i \"$MOVIE\" -vf \"select='between(t,235,238)+between(t,490,493)+between(t,870,873)+between(t,1200,1203)',setpts=N/FRAME_RATE/TB,scale=640:-2\"  -an \"$OUT_FILEPATH\""
